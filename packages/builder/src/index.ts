/**
 * @msf/builder — MSF Package Builder
 *
 * Tools for building and managing MSF packages for consumption.
 * Includes inventory management, package scaffolding, auto-generation,
 * and utilities for creating distributable MSF packages.
 */
...
export class Inventory {
  ...
  /**
   * Get a sample by ID (for compiler compatibility)
   * Converts InventoryEntry to the Sample format expected by the compiler
   */
  getSample(id: string): {
    id: string;
    path: string;
    ...
    return {
      ...
      metadata: {
        ...
        duration: entry.metadata.audio.duration,
        sampleRate: entry.metadata.audio.sampleRate,
        channels: entry.metadata.audio.channels,
        format: entry.metadata.audio.format,
        roundRobin: entry.metadata.roundRobin,
      },
    };
  }
  ...
  /**
   * List all samples (for compiler compatibility)
   */
  listSamples(): Array<{
    id: string;
    path: string;
...
// Re-export MSF loader utilities
export {
  loadMSFFromPackage,
...
// Re-export auto-generation utilities
export {
  autoGenerateMSF,
  enhanceInventoryFromFilenames,
} from "./auto-msf.js";
